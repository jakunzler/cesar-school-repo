// Conta Web UI padrão (username: admin, password: 1423).
// Fonte: Open5GS v2.7.7 docs/assets/webui/mongo-init.js
// https://raw.githubusercontent.com/open5gs/open5gs/v2.7.7/docs/assets/webui/mongo-init.js
//
// Uso: mongosh open5gs scripts/mongo-webui-default-admin.js

db = db.getSiblingDB('open5gs');

const doc = {
  salt: 'f5c15fa72622d62b6b790aa8569b9339729801ab8bda5d13997b5db6bfc1d997',
  hash: '',
  username: 'admin',
  roles: ['admin'],
  __v: 0,
};

const r = db.accounts.replaceOne({ username: 'admin' }, doc, { upsert: true });
print(JSON.stringify({ ok: r.acknowledged, matched: r.matchedCount, modified: r.modifiedCount, upserted: r.upsertedCount }));
