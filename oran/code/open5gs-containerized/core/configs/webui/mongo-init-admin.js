// Script de inicialização do usuário admin do WebUI Open5GS
// Credenciais padrão: admin / 1423
// Fonte: https://github.com/open5gs/open5gs/blob/main/docs/assets/webui/mongo-init.js

db = db.getSiblingDB('open5gs');
if (db.accounts.countDocuments({}) === 0) {
  db.accounts.insertOne({
    salt: 'f5c15fa72622d62b6b790aa8569b9339729801ab8bda5d13997b5db6bfc1d997',
    hash: '',
    username: 'admin',
    roles: ['admin'],
    __v: 0
  });
  print('Admin user created: admin / 1423');
} else {
  print('Admin user already exists');
}
