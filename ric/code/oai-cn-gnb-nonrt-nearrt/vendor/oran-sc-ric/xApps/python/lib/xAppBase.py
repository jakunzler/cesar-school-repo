...
class xAppBase(object):
    ...
    def subscribe(self, e2_node_id, ran_function_id, event_trigger_def, action_def, indication_callback, e2sm_type=e2sm_types.E2SM_UNKNOWN):
        ...
        # Aguardar mapeamento REST SubscriptionId -> E2EventInstanceId (usado no RMR)
        for _ in range(100):
            if subscription_id not in self.my_subscriptions:
                break
            time.sleep(0.02)
                ...
                if (summary['message type'] == 12050):
                    ...
                    try:
                        ...
                        if subscriptionObj is None:
                            subscriptionObj = self.my_subscriptions.get(str(E2EventInstanceId), None)
                        if subscriptionObj is None:
