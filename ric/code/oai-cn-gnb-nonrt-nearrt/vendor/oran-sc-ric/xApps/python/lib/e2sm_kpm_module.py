...
def parse_kpm_timestamp(raw):
    """Converte E2SM-KPM TimeStamp para datetime UTC.

    OAI (KPM v2.03): OCTET STRING 4 bytes = Unix seconds.
    O-RAN SC / srsRAN (v4): OCTET STRING 8 bytes = NTP 64-bit.
    """
    if raw is None:
        return None

    if isinstance(raw, int):
        if 0 < raw < 1_000_000_000_000:
            return datetime.datetime.utcfromtimestamp(raw)
        if raw < 1_000_000_000_000_000:
            return datetime.datetime.utcfromtimestamp(raw / 1_000_000.0)
        return ntp_ts_to_datetime(raw)

    if isinstance(raw, (bytes, bytearray)):
        if len(raw) == 4:
            # OAI v2.03 chega neste lab como Unix seconds little-endian. Mantemos
            # fallback big-endian para não quebrar capturas vindas de outros stacks.
            now = datetime.datetime.utcnow()
            candidates = [
                datetime.datetime.utcfromtimestamp(int.from_bytes(raw, byteorder='little', signed=False)),
                datetime.datetime.utcfromtimestamp(int.from_bytes(raw, byteorder='big', signed=False)),
            ]
            return min(candidates, key=lambda dt: abs((dt - now).total_seconds()))
        if len(raw) == 8:
            value = int.from_bytes(raw, byteorder='big', signed=False)
            # 8 bytes: NTP (srsRAN) ou Unix µs (KPM v3 OAI, se usado no futuro)
            if value > (2208988800 << 32):
                return ntp_ts_to_datetime(value)
            if value > 1_000_000_000_000:
                return datetime.datetime.utcfromtimestamp(value / 1_000_000.0)
            return datetime.datetime.utcfromtimestamp(value)

    return None

def _parse_meas_record_value(meas_record_item):
    """Normaliza MeasurementRecordItem (integer/real/noValue) do asn1tools."""
    if not meas_record_item:
        return None
    value_type = meas_record_item[0]
    value = meas_record_item[1] if len(meas_record_item) > 1 else None
    if value_type in ('noValue', 'notChosen'):
        return None
    if value_type in ('integer', 'real', 'valueInt', 'valueReal'):
        return value
    return value
...
class e2sm_kpm_module(object):
    ...
    def subscribe_report_service_style_4_oai(self, e2_node_id, reportingPeriod, metric_names, granulPeriod, indication_callback,
                                             sst=222, sd=123):
        """Style 4 com matching S-NSSAI (único suportado pelo agente E2 OAI)."""
        matchingUeConds = [{'testCondInfo': {
            'testType': ('sNSSAI', 'true'),
            'testExpr': 'equal',
            'testValue': ('valueOctS', bytes([sst, (sd >> 16) & 0xFF, (sd >> 8) & 0xFF, sd & 0xFF])),
        }}]
        self.subscribe_report_service_style_4(
            e2_node_id, reportingPeriod, matchingUeConds, metric_names, granulPeriod, indication_callback)
    ...
    def extract_hdr_info(self, indication_hdr):
        dt = parse_kpm_timestamp(indication_hdr.get('colletStartTime'))
        if dt is not None:
            indication_hdr['colletStartTime'] = dt.strftime('%Y-%m-%d %H:%M:%S UTC')
    ...
    def _extract_meas_data_ind_msg_f1(self, indication_msg_content):
        ...
        for measDataItem in measData:
            ...
            for measRecordItem in measRecord:
                value = _parse_meas_record_value(measRecordItem)
