...
static kpm_ric_ind_hdr_format_1_t kpm_ind_hdr_frm_1(void)
{
  ...
  // PrintableString no decoder O-RAN SC rejeita '_' em get_ngran_name() (ex.: ngran_gNB).
  static const char kpm_sender_name[] = "OAI-gNB";
  hdr_frm_1.sender_name = calloc(1, sizeof(byte_array_t));
  *hdr_frm_1.sender_name = cp_str_to_ba(kpm_sender_name);
